...
> ✅ **Stable.** The certificate-based security backbone, agent onboarding, and the admin UI are implemented end to end, live-verified against a real running server, and covered by an automated test suite. Real Windows Update installation (including selective, per-update installs), the Windows installer's install/uninstall behavior, both platforms' remote reboot scheduling, Windows Event Log output, the Linux RPM/`dnf`/`yum` update path, and the agent's own `.deb`/`.rpm` packaging install/upgrade have since all been confirmed against real hosts too. A few pieces (Windows-on-ARM entirely, the Linux agent's own self-update *apply* step, and real arm64 hardware specifically — the arm64 packaging confirmation above used QEMU emulation) are still not verified against real target hardware — see [Project status](#-project-status) below before relying on those specifically.
...
### 🔑 Login & access
- Local `admin` account (random strong password on first start) and optional Active Directory login (LDAP bind, gated on group membership) — both cookie-session based, with configurable brute-force protection and a trusted-IP exemption. The `PUT /api/auth/password` endpoint to change the local admin password exists and is fully functional, but there's no page for it in the admin UI yet — see below for how to change it today.
...
UpdateWatch2 was built with **vibe coding**: implemented and iterated on with [Claude Code](https://claude.com/claude-code) (Anthropic) in conversation, rather than hand-written line by line, driven by a human-authored architecture brief. Code has been built, tested, and repeatedly run live at each step — not just compiled — and the mutual-TLS security backbone (registration, approval, renewal, re-issuance, root rotation), the admin UI, and the agent-server protocol are implemented end to end, live-verified, and covered by an automated test suite (server: xUnit; UI: Vitest). The `apt`-based Linux update path has also been live-verified against a real package cache, and the `dnf`/`yum` RPM-family path (Fedora's dnf5, Rocky Linux's classic dnf4, and CentOS 7's bare legacy yum — search, reboot detection, and install) has since been fully confirmed too, closing `updatewatch2-agent#12`. The Windows Update API (WUApiLib COM) integration's real search→download→install cycle and the NSIS installer's actual install/uninstall through a real Service Control Manager have since been confirmed too, against a real Windows host, including a selective (per-update) install — closing `updatewatch2-agent#13`. Both platforms' remote reboot scheduling, Windows Event Log output, and the Windows agent's self-update apply step (silently re-running the installer) have likewise been confirmed against real hosts. The agent's own `.deb`/`.rpm` packaging (a fresh install, an upgrade correctly preserving an already-configured `agent.conf`, and a clean uninstall, through real `dpkg`/`rpm`) has since been confirmed on both x86_64 (natively) and arm64 (under QEMU emulation), closing `updatewatch2-agent#9`. A number of pieces are explicitly **not yet live-verified against a real target host** — only unit/integration-tested, or design-reviewed but never run live — and are called out as such in code comments:

- **Windows**: Windows-on-ARM (`win-arm64`) entirely — no ARM64 device has ever been available to test it on.
- **Linux**: pre-downloading `apt` updates before an install trigger specifically (the `dnf`/`yum` side is now confirmed too — only the argument-building logic has real test coverage for `apt`); the Linux agent's own self-update *apply* step (`dpkg -i`/`rpm -U` inside a detached `systemd-run` scope) completing end to end on a real host — `updatewatch2-agent#24` tracks a real production report suggesting it may still fail silently, distinct from the packaging install/upgrade path above.
- **Packaging**: the packaging scripts' `systemctl enable`/`start`/`stop` branches specifically — every install/upgrade/uninstall test so far ran against a minimal container image with no `systemctl` binary at all, so only the real production `hpn54l` host (`.deb` only) has ever exercised that branch for real. The arm64 packaging confirmation above was under QEMU emulation, not real arm64 hardware; the multi-arch Docker image (a separate, server-side concern) likewise has not been pulled/run on real arm64 hardware.
- **Selective update install**'s remaining Linux-side gap is `apt --only-upgrade` specifically — dnf/yum's own scoped `update` is now confirmed, per above; the Windows-side WUA COM filtering is confirmed too.

Everything else — including the full mutual-TLS security backbone, agent onboarding/approval/renewal/re-issuance/CA root rotation, the admin UI, the Windows Update integration and installer behavior, both platforms' remote reboot scheduling, the Linux `dnf`/`yum` update path, and the agent's own packaging install/upgrade path above — has been confirmed working end to end against a real running deployment. Keep backups of the `/app/data` and `/app/certs` volumes.
...
Then open **http://localhost:8795** and log in as `admin` — the randomly generated first-start password is printed to the container's log (`docker logs updatewatch2-server`). There's no change-password page in the admin UI yet; the endpoint (`PUT /api/auth/password`) exists and works, just isn't wired into any UI. Two ways to actually change it today: set `UPDATEWATCH2_RESET_ADMIN_PASSWORD` (see below) and restart, or call the endpoint directly while logged in, e.g.:

```bash
curl -X PUT http://localhost:8795/api/auth/password \
  -H 'Content-Type: application/json' \
  -b cookies.txt \
  -d '{"currentPassword":"<current>","newPassword":"<new, >=16 chars, mixed case + digit + symbol>"}'
```

(`cookies.txt` from a prior `curl -c cookies.txt -X POST .../api/auth/login ...` — or reuse your browser's session cookie.)
...
| `UPDATEWATCH2_TRUSTED_PROXY` | | — (trusts any peer) | Restricts the reverse-proxy allow-list to specific IP(s)/CIDR(s) (comma-separated) instead of trusting any directly-connecting peer for `X-Forwarded-For`/`X-Forwarded-Proto`. Optional hardening for deployments with a known, stable proxy IP — see `.env.example`. A malformed value is logged and ignored, never crashes startup. |
