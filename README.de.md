...
> ✅ **Stabil.** Das zertifikatsbasierte Sicherheitsfundament, das Agent-Onboarding und die Admin-Oberfläche sind durchgängig implementiert, gegen einen echten laufenden Server verifiziert und durch eine automatisierte Testsuite abgedeckt. Die echte Windows-Update-Installation (inklusive selektiver Installation einzelner Updates) sowie das Installations-/Deinstallationsverhalten des Windows-Installers wurden inzwischen ebenfalls gegen einen echten Windows-Host bestätigt. Die Planung eines Remote-Neustarts auf beiden Plattformen, die Windows-Ereignisprotokoll-Ausgabe sowie der Linux-RPM-/`dnf`-/`yum`-Update-Pfad wurden inzwischen ebenfalls gegen echte Hosts bestätigt. Einige wenige Teile (Windows-on-ARM vollständig, der Selfupdate-Anwendungsschritt des Linux-Agents sowie der arm64-Paketierungs-Installations-/Upgrade-Pfad unter Linux — nur unter QEMU-Emulation bestätigt, nicht auf echter arm64-Hardware) wurden noch nicht gegen echte Ziel-Hardware verifiziert — siehe [Projektstatus](#-projektstatus) weiter unten, bevor du dich speziell darauf verlässt.
