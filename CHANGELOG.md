...
- **An admin can now remotely restart an agent's own service process from the admin UI, at the user's explicit request ("Es fehlt noch die Funktion um den Client über die Oberfläche neu starten zu können") — protocol `0.11.0`, DB schema `0.16.0`.** A distinct agent-lifecycle action, deliberately never conflated with an OS reboot or an OS-update install (CLAUDE.md's "update installation never triggers a reboot itself" rule is unrelated — this restarts only the agent's own service, nothing else, on the target machine). `AgentDetailPage` gained a "Restart agent" button (confirm-gated, since it briefly interrupts management connectivity) and its own status card, mirroring the existing install-trigger/install-status pattern exactly. New `Agent.PendingRestartRequestedAt`/`LastRestartOutcome`/`LastRestartErrorDetail`/`LastRestartCompletedAt` columns; `IAgentService.TriggerRestartAsync`/`AcknowledgeRestartAsync` mirror `Updates.IUpdateService`'s existing `TriggerInstallAsync`/`AcknowledgeInstallAsync` field-for-field, including the same fire-and-forget delivery (the request is surfaced to the agent as an additive `restartRequested` field on the existing `alive` heartbeat response, picked up on the agent's own next poll, never pushed). `POST /api/agents/{hostname}/restart` (admin-session-gated, alongside Approve/Reissue/Delete on `AgentsController`) triggers it; `POST /api/agents/{hostname}/restart-ack` (mTLS-gated, on `AgentProtocolController` rather than `AgentsController` — that controller's class-level `[Authorize]` is the cookie-session scheme, which an agent's client certificate could never also satisfy on the same request) is the agent's acknowledgement. New `RestartOutcome` enum (`[JsonConverter(JsonStringEnumConverter)]`, mirroring `Updates.InstallOutcome`'s own established reasoning for why the default numeric encoding isn't used for a wire-facing enum in this codebase) kept deliberately separate from `InstallOutcome` even though the shape is identical, matching this codebase's existing checker-facing/wire-facing DTO layering.
- Real test coverage: `AgentServiceTests` (trigger/acknowledge, including the audit-log-actor and stale-error-detail-clearing cases already established for the sibling install feature) and `ApiEndpointTests` (admin-session-required, not-found, and the mTLS-gate-exists-even-though-`WebApplicationFactory`-can't-exercise-real-TLS case — same standing limitation this file's own `Renew_rejects_a_request_with_no_client_certificate` test already documents). Web: `AgentDetailPage.test.tsx` covers the trigger/confirm/decline/pending/outcome/error-detail cases, mirroring the existing install-trigger test suite.
