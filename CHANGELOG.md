...
- **Optional trusted-reverse-proxy allow-list, at the user's direct question — "What's the name of the security mechanism that decides which reverse proxy is allowed and blocks all others, and would it make sense to add here?"** ASP.NET Core's Forwarded Headers Middleware has exactly this mechanism (`ForwardedHeadersOptions.KnownProxies`/`KnownIPNetworks`), but `Program.cs` deliberately clears both — a considered trade-off, not an oversight, since this server ships as a generic image behind an arbitrary, unknown-at-build-time reverse proxy, and a *mandatory* restriction risked silently reintroducing the exact bug `CookieSecurePolicy.SameAsRequest` was built to fix (the auth cookie silently never stored over plain HTTP, no error shown). The one genuinely dangerous spoofing vector this leaves open (`X-Forwarded-For` bypassing `UPDATEWATCH2_TRUSTEDIP`'s brute-force exemption) is already closed via `Auth/RealRemoteIpAccessor` — what remained unrestricted (the auth cookie's `Secure` flag via `X-Forwarded-Proto`, and the audit-log actor IP, already an accepted non-security use) doesn't enable a cross-user attack, just low-severity.
  - New `UPDATEWATCH2_TRUSTED_PROXY` (comma-separated IPs/CIDR ranges, e.g. `10.0.5.4,172.20.0.0/16`) — **optional, off by default, does not change default behavior**. When set, restores the normal ASP.NET Core allow-list for admins who know their reverse proxy's stable IP (e.g. a Compose sibling container, a static host-level nginx). New `Auth/TrustedProxyOptionsConfigurator.TryApply` parses the value (routing bare IPs to `KnownProxies`, CIDR entries to `KnownIPNetworks` via the built-in `System.Net.IPNetwork.TryParse`) and validates the whole value atomically — one malformed entry rejects the entire list rather than silently dropping just that entry, logging an error and leaving the allow-list exactly as it was (today's default), never crashing startup over a typo in an opt-in hardening knob.
  - `Program.cs`'s existing `ForwardedHeadersOptions` setup switched from `Configure<T>(Action<T>)` to `AddOptions<T>().Configure<ILoggerFactory>(...)`, so a malformed value can actually be logged (this now resolves lazily, well after the DI container exists, unlike this file's other early pre-`Build()` env var reads).
  - `.env.example`, `docker/docker-compose.yml`, and both READMEs' env-var tables updated to match, mirroring the existing `UPDATEWATCH2_TRUSTEDIP` documentation.
  - New `tests/.../Auth/TrustedProxyOptionsConfiguratorTests.cs` (pure parser unit tests: single IP, single CIDR, comma-separated mix, IPv6, whitespace, blank/no-op, and — the important one — that a malformed entry anywhere in the value rejects the *whole* list atomically) plus `tests/.../Api/TrustedProxyEnvironmentVariableTests.cs` (a real end-to-end test proving the actual `Program.cs` wiring — a request from an untrusted peer has its `X-Forwarded-Proto` ignored while one from the configured trusted peer has it honored, over a single shared `WebApplicationFactory<Program>`). `Configure<ForwardedHeadersOptions>` resolves once per host build and is cached, unlike `UPDATEWATCH2_TRUSTEDIP`'s own live per-request env read (which is why `AuthControllerTests` can toggle that one per-`[Fact]` against its shared fixture but this couldn't) — deliberately still only ONE ad-hoc factory, not one per case: this project has no existing precedent for multiple ad-hoc `WebApplicationFactory<Program>` instances in the same run, and `Program.cs`'s own `ConfigureKestrel`/`ListenAnyIP` calls run unconditionally even under test — an earlier two-factory version of this test produced a real, if intermittent, cross-test failure cascade (57 unrelated failures) in one full-suite run out of several, not reproduced after consolidating to one factory across three subsequent full runs. Uses `TestServer.SendAsync(Action<HttpContext>)` instead of a plain `HttpClient` request, since `WebApplicationFactory`'s in-memory test host never sets `Connection.RemoteIpAddress` and ASP.NET Core's allow-list check is skipped entirely whenever that's null, so a normal test request could never actually exercise the allow-list either way.
  - Two real environment gotchas surfaced building this (see CLAUDE.md's "X-Forwarded-For spoofing" bullet for the full detail): `ForwardedHeadersOptions` actually lives in `Microsoft.AspNetCore.Builder` in this ASP.NET Core version, not the historical `Microsoft.AspNetCore.HttpOverrides` namespace still used elsewhere in `Program.cs` (only resolves there via a compile-time forward that needs `Microsoft.NET.Sdk.Web`, confirmed not to work reliably from the test project even with an explicit `FrameworkReference`); and `ForwardedHeadersOptions`'s own constructor pre-populates `KnownProxies` with the IPv6 loopback address, so tests constructing one directly must `Clear()` both lists first to match `Program.cs`'s real usage.
