...
UpdateWatch2 wurde per **Vibe-Coding** entwickelt: implementiert und iteriert im Dialog mit [Claude Code](https://claude.com/claude-code) (Anthropic), statt Zeile für Zeile von Hand geschrieben, angetrieben von einem menschlich verfassten Architektur-Briefing. Der Code wurde bei jedem Schritt gebaut, getestet und wiederholt live ausgeführt — nicht nur kompiliert — und das zertifikatsbasierte Sicherheitsfundament (Registrierung, Freigabe, Erneuerung, Neuausstellung, Wurzel-Rotation), die Admin-Oberfläche sowie das Agent-Server-Protokoll sind durchgängig implementiert, live verifiziert und durch eine automatisierte Testsuite abgedeckt (Server: xUnit; Oberfläche: Vitest). Auch der `apt`-basierte Linux-Update-Pfad wurde live gegen einen echten Paket-Cache verifiziert. Der komplette Such-→Download-→Installations-Zyklus der Windows-Update-API-Integration (WUApiLib-COM) sowie das tatsächliche Installations-/Deinstallationsverhalten des NSIS-Installers über eine echte Dienststeuerung wurden inzwischen ebenfalls gegen einen echten Windows-Host bestätigt, inklusive einer selektiven (Pro-Update-)Installation — womit `updatewatch2-agent#13` geschlossen wurde. Eine Reihe von Teilen ist ausdrücklich **noch nicht gegen ein echtes Zielsystem live verifiziert** — nur unit-/integrationsgetestet, oder konzeptionell durchdacht, aber nie live ausgeführt — und im Code entsprechend gekennzeichnet:

- **Windows**: der Windows-Selfupdate-Anwendungspfad des Agents (stilles erneutes Ausführen des Installers); die `shutdown.exe`-basierte Planung eines Remote-Neustarts; die Windows-Ereignisprotokoll-Ausgabe; Windows-on-ARM (`win-arm64`) vollständig — dafür stand nie ein echtes ARM64-Gerät zur Verfügung.
...
- **Paketierung**: `.rpm`-Installation/-Upgrade (nur strukturell geprüft, nie über einen echten Paketmanager ausgeführt); das Multi-Arch-Docker-Image wurde nie auf echter arm64-Hardware gezogen/ausgeführt; der arm64-Installationspfad unter Linux selbst ist ebenso auf echter Hardware unbestätigt.
- Die Linux-seitigen Betriebssystem-Befehle der **selektiven Update-Installation** (apt `--only-upgrade`, dnf/yum-eingeschränktes `update`) — nur die umgebende Auswahl-/Orchestrierungslogik ist dort live verifiziert; die Windows-seitige WUA-COM-Filterung ist wie oben beschrieben bestätigt.

Alles andere — einschließlich des vollständigen zertifikatsbasierten Sicherheitsfundaments, des Agent-Onboardings/der Freigabe/Erneuerung/Neuausstellung/CA-Wurzel-Rotation, der Admin-Oberfläche sowie nun auch der Windows-Update-Integration und des Installer-Verhaltens oben — wurde durchgängig gegen eine echte laufende Instanz bestätigt. Lege regelmäßig Backups der Volumes `/app/data` und `/app/certs` an.
