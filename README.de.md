...
> ✅ **Stabil.** Das zertifikatsbasierte Sicherheitsfundament, das Agent-Onboarding und die Admin-Oberfläche sind durchgängig implementiert, gegen einen echten laufenden Server verifiziert und durch eine automatisierte Testsuite abgedeckt. Die echte Windows-Update-Installation (inklusive selektiver Installation einzelner Updates) sowie das Installations-/Deinstallationsverhalten des Windows-Installers wurden inzwischen ebenfalls gegen einen echten Windows-Host bestätigt. Die Planung eines Remote-Neustarts auf beiden Plattformen, die Windows-Ereignisprotokoll-Ausgabe, der Linux-RPM-/`dnf`-/`yum`-Update-Pfad sowie die eigene `.deb`/`.rpm`-Paketierungs-Installation/-Upgrade des Agents wurden inzwischen ebenfalls gegen echte Hosts bestätigt. Einige wenige Teile (Windows-on-ARM vollständig, der Selfupdate-Anwendungsschritt des Linux-Agents sowie speziell echte arm64-Hardware — die arm64-Paketierungsbestätigung oben lief unter QEMU-Emulation) wurden noch nicht gegen echte Ziel-Hardware verifiziert — siehe [Projektstatus](#-projektstatus) weiter unten, bevor du dich speziell darauf verlässt.
...
UpdateWatch2 wurde per **Vibe-Coding** entwickelt: implementiert und iteriert im Dialog mit [Claude Code](https://claude.com/claude-code) (Anthropic), statt Zeile für Zeile von Hand geschrieben, angetrieben von einem menschlich verfassten Architektur-Briefing. Der Code wurde bei jedem Schritt gebaut, getestet und wiederholt live ausgeführt — nicht nur kompiliert — und das zertifikatsbasierte Sicherheitsfundament (Registrierung, Freigabe, Erneuerung, Neuausstellung, Wurzel-Rotation), die Admin-Oberfläche sowie das Agent-Server-Protokoll sind durchgängig implementiert, live verifiziert und durch eine automatisierte Testsuite abgedeckt (Server: xUnit; Oberfläche: Vitest). Auch der `apt`-basierte Linux-Update-Pfad wurde live gegen einen echten Paket-Cache verifiziert, und der RPM-Familien-Pfad `dnf`/`yum` (Fedoras dnf5, das klassische dnf4 von Rocky Linux sowie das reine, klassische yum auf CentOS 7 — Suche, Neustart-Erkennung und Installation) wurde inzwischen ebenfalls vollständig bestätigt, womit `updatewatch2-agent#12` geschlossen wurde. Der komplette Such-→Download-→Installations-Zyklus der Windows-Update-API-Integration (WUApiLib-COM) sowie das tatsächliche Installations-/Deinstallationsverhalten des NSIS-Installers über eine echte Dienststeuerung wurden inzwischen ebenfalls gegen einen echten Windows-Host bestätigt, inklusive einer selektiven (Pro-Update-)Installation — womit `updatewatch2-agent#13` geschlossen wurde. Die Planung eines Remote-Neustarts auf beiden Plattformen, die Windows-Ereignisprotokoll-Ausgabe sowie der Windows-Selfupdate-Anwendungsschritt des Agents (stilles erneutes Ausführen des Installers) wurden ebenfalls gegen echte Hosts bestätigt. Auch die eigene `.deb`/`.rpm`-Paketierung des Agents (eine frische Installation, ein Upgrade mit korrekt erhaltener `agent.conf` sowie eine saubere Deinstallation, jeweils über echtes `dpkg`/`rpm`) wurde inzwischen sowohl auf x86_64 (nativ) als auch arm64 (unter QEMU-Emulation) bestätigt, womit `updatewatch2-agent#9` geschlossen wurde. Eine Reihe von Teilen ist ausdrücklich **noch nicht gegen ein echtes Zielsystem live verifiziert** — nur unit-/integrationsgetestet, oder konzeptionell durchdacht, aber nie live ausgeführt — und im Code entsprechend gekennzeichnet:

- **Windows**: Windows-on-ARM (`win-arm64`) vollständig — dafür stand nie ein echtes ARM64-Gerät zur Verfügung.
- **Linux**: das Vorab-Herunterladen von `apt`-Updates vor einem Installations-Trigger speziell (die dnf/yum-Seite ist inzwischen ebenfalls bestätigt — nur für apt hat die Argument-Konstruktion echte Testabdeckung); der eigene Selfupdate-Anwendungsschritt des Linux-Agents (`dpkg -i`/`rpm -U` innerhalb eines abgekoppelten `systemd-run`-Scopes), der vollständig gegen einen echten Host durchläuft — `updatewatch2-agent#24` dokumentiert einen echten Produktionsbericht, der auf ein mögliches stilles Fehlschlagen hindeutet, zu unterscheiden vom oben bestätigten Paketierungs-Installations-/Upgrade-Pfad.
- **Paketierung**: konkret die `systemctl enable`/`start`/`stop`-Zweige der Paketierungsskripte — jeder bisherige Installations-/Upgrade-/Deinstallationstest lief gegen ein minimales Container-Image ganz ohne `systemctl`-Binary, sodass nur der echte Produktions-Host `hpn54l` (nur `.deb`) diesen Zweig je real durchlaufen hat. Die arm64-Bestätigung oben lief unter QEMU-Emulation, nicht auf echter arm64-Hardware; das Multi-Arch-Docker-Image (ein separates, serverseitiges Thema) wurde ebenso nie auf echter arm64-Hardware gezogen/ausgeführt.
- Der verbleibende Linux-seitige Punkt bei der **selektiven Update-Installation** ist `apt --only-upgrade` speziell — dnf/yums eigenes eingeschränktes `update` ist wie oben beschrieben bestätigt; die Windows-seitige WUA-COM-Filterung ist ebenfalls bestätigt.

Alles andere — einschließlich des vollständigen zertifikatsbasierten Sicherheitsfundaments, des Agent-Onboardings/der Freigabe/Erneuerung/Neuausstellung/CA-Wurzel-Rotation, der Admin-Oberfläche, der Windows-Update-Integration und des Installer-Verhaltens, der Planung eines Remote-Neustarts auf beiden Plattformen, des Linux-`dnf`/`yum`-Update-Pfads sowie der eigenen Paketierungs-Installation/-Upgrade des Agents oben — wurde durchgängig gegen eine echte laufende Instanz bestätigt. Lege regelmäßig Backups der Volumes `/app/data` und `/app/certs` an.

## 🐳 Installation & Konfiguration (Docker)

UpdateWatch2 Server wird als ein einzelnes, in sich geschlossenes Docker-Image ausgeliefert — API und gebaute Admin-Oberfläche in einem Container, kein separater Build-Schritt oder Datenbank-Container nötig. Das veröffentlichte Image ist multi-arch (`linux/amd64` und `linux/arm64`, z. B. AWS Graviton, Ampere Altra, Raspberry Pi 4/5) — Docker/Podman ziehen automatisch das zur Host-Architektur passende Manifest, ohne separaten Tag oder Flag.
...
| `UPDATEWATCH2_TRUSTED_PROXY` | | — (vertraut jedem Peer) | Beschränkt die Reverse-Proxy-Allowlist auf bestimmte IP(s)/CIDR-Bereiche (kommagetrennt) statt `X-Forwarded-For`/`X-Forwarded-Proto` von jedem direkt verbundenen Peer zu vertrauen. Optionale Härtung für Deployments mit bekannter, stabiler Proxy-IP — siehe `.env.example`. Ein fehlerhafter Wert wird geloggt und ignoriert, führt nie zum Startabbruch. |
