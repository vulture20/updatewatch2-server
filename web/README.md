...
- `src/pages/` — `LoginPage` (wired to `/api/auth/login`), `AgentsListPage` (with bulk approve), `AgentDetailPage`, `AdminPage`
...
- Only the local `admin` account can log in — AD-authenticated login (`updatewatch2-server#2`) is a separate, not-yet-implemented path.
