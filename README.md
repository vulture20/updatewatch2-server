...
> ✅ **Stable.** The certificate-based security backbone, agent onboarding, and the admin UI are implemented end to end, live-verified against a real running server, and covered by an automated test suite. Real Windows Update installation (including selective, per-update installs) and the Windows installer's install/uninstall behavior have since been confirmed against a real Windows host too. A few pieces (the RPM/dnf update path, and the arm64 install path on both Windows and Linux) are still not verified against a real target host — see [Project status](#-project-status) below before relying on those specifically.
...
UpdateWatch2 was built with **vibe coding**: implemented and iterated on with [Claude Code](https://claude.com/claude-code) (Anthropic) in conversation, rather than hand-written line by line, driven by a human-authored architecture brief. Code has been built, tested, and repeatedly run live at each step — not just compiled — and the mutual-TLS security backbone (registration, approval, renewal, re-issuance, root rotation), the admin UI, and the agent-server protocol are implemented end to end, live-verified, and covered by an automated test suite (server: xUnit; UI: Vitest). The `apt`-based Linux update path has also been live-verified against a real package cache. The Windows Update API (WUApiLib COM) integration's real search→download→install cycle and the NSIS installer's actual install/uninstall through a real Service Control Manager have since been confirmed too, against a real Windows host, including a selective (per-update) install — closing `updatewatch2-agent#13`. A number of pieces are explicitly **not yet live-verified against a real target host** — only unit/integration-tested, or design-reviewed but never run live — and are called out as such in code comments:

- **Windows**: the agent's Windows self-update apply path (silently re-running the installer); `shutdown.exe`-based remote reboot scheduling; Windows Event Log output; Windows-on-ARM (`win-arm64`) entirely — no ARM64 device has been available to test it on.
- **Linux**: the RPM/`dnf`/`yum` update path (only `apt` was verified against a real package cache); pre-downloading updates before an install trigger (only the argument-building logic has real test coverage); `systemd`-based remote reboot scheduling (never run for real — would reboot the shared sandbox this project's own tooling depends on).
- **Packaging**: `.rpm` install/upgrade (structural inspection only, never run through a real package manager); the multi-arch Docker image has not been pulled/run on real arm64 hardware; the arm64 Linux install path itself is likewise unconfirmed on real hardware.
- **Selective update install**'s Linux-side OS-level install commands (apt `--only-upgrade`, dnf/yum scoped `update`) — only the surrounding selection/orchestration logic is live-verified there; the Windows-side WUA COM filtering is confirmed, per above.

Everything else — including the full mutual-TLS security backbone, agent onboarding/approval/renewal/re-issuance/CA root rotation, the admin UI, and now the Windows Update integration and installer behavior above — has been confirmed working end to end against a real running deployment. Keep backups of the `/app/data` and `/app/certs` volumes.
