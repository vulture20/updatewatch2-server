...
> ✅ **Stable.** The certificate-based security backbone, agent onboarding, and the admin UI are implemented end to end, live-verified against a real running server, and covered by an automated test suite. Real Windows Update installation (including selective, per-update installs), the Windows installer's install/uninstall behavior, both platforms' remote reboot scheduling, Windows Event Log output, and the Linux RPM/`dnf`/`yum` update path have since all been confirmed against real hosts too. A few pieces (Windows-on-ARM entirely, the Linux agent's own self-update *apply* step, and the Linux arm64 packaging install/upgrade path — confirmed only under QEMU emulation, not real arm64 hardware) are still not verified against real target hardware — see [Project status](#-project-status) below before relying on those specifically.
...
Everything else — including the full mutual-TLS security backbone, agent onboarding/approval/renewal/re-issuance/CA root rotation, the admin UI, the Windows Update integration and installer behavior, both platforms' remote reboot scheduling, and the Linux `dnf`/`yum` update path above — has been confirmed working end to end against a real running deployment. Keep backups of the `/app/data` and `/app/certs` volumes.
